...
## Which samples are driving the PCA clustering?

```{r}
PCA_label <- function(data, title){
  m <- filter(data, sex=='male')
  len <- length(unique(data$familyname))
  males <- length(unique(m$ID))
   ...
   names(m_fram)<-unique(m$familyname)
   m_matrixcont<-matrix(as.vector(m$country),nrow=males,ncol=len,byrow=T)
   m_matrixpop<-matrix(as.vector(m$ID),nrow=males,ncol=len,byrow=T)
   m_frampop<-data.frame(m_matrixpop)
   m_framcont<-data.frame(m_matrixcont)
   m_contcol<-c(m_framcont$X1)
   m_popcol<-c(m_frampop$X1)

   mHGDP.pca <- prcomp(m_fram, center = TRUE, scale = TRUE)

    library(ggbiplot)
    (m_PCA <- ggbiplot(mHGDP.pca, var.axes=FALSE, groups = m_contcol, ellipse = FALSE)+ ggtitle("Males")+ theme(plot.title = element_text(size = 8, hjust = 0.5)) + geom_text(aes(x=mHGDP.pca$x[,1],y=mHGDP.pca$x[,2], label=m_popcol), size=2))
 }
```

```{r}
PCA_label(TE_cutoff, "All the repetitive sequences")
```
...
PCA_otherPC(TE_cutoff, "All the repetitive sequences", 1, 4)
```