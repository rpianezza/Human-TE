...
knitr::opts_knit$set(root.dir = '/Volumes/Temp1/rpianezza/SGDP/SNP/')
```
...
lev = c("Africa", "America", "Central Asia and Siberia", "East Asia", "South Asia", "West Eurasia", "Oceania")

f <- f_pca_result$x %>% as_tibble() %>% add_column(.before = 1, ID=f_metadata$ID, sex=f_metadata$sex, pop=f_metadata$pop, country=f_metadata$country) %>% mutate(country= relevel(as.factor(country), ref=lev)) %>% as.data.frame() %>%
...
labs(x=paste0("PC1: ",round(f_var_explained[1]*100,1),"%"),
 ...
 theme(plot.title = element_text(hjust = 0.5))
  ...
  m <- m_pca_result$x %>% as_tibble() %>% add_column(.before = 1, ID=m_metadata$ID, sex=m_metadata$sex, pop=m_metadata$pop, country=m_metadata$country) %>% mutate(country= relevel(as.factor(country), ref=lev)) %>% as.data.frame() %>%
  ggplot(aes(x=PC1,y=PC2, color=country)) + geom_point(size=2) +
...
```{r all}
SNP_all("/Volumes/Temp1/rpianezza/SGDP/SNP/separed.all.08.5000x.matrix.tsv", SGDP)
```
