...
```{r}
cn <- HGDP_clean %>% filter(type=="te") %>% group_by(familyname) %>% summarise(mean = mean(copynumber)) %>% filter(mean < 100) %>% select(familyname) %>% pull()

(variability <- classification %>% filter(familyname %in% cn, !(class%in%c("satellite", NA))) %>% group_by(pop, sex, familyname, country) %>% summarise(mean_cn = mean(copynumber), variance = var(copynumber), coeff_var = variance/mean_cn, individuals = n()) %>% ungroup())

(pop <- variability %>% group_by(pop, sex, country, individuals) %>% summarise(mean_cv = mean(coeff_var, na.rm=TRUE), normalized_cv = mean_cv/individuals) %>% arrange(desc(mean_cv)) %>% distinct())

(country <- pop %>% group_by(country, sex) %>% summarise(mean_cv = mean(mean_cv, na.rm = TRUE), individuals=sum(individuals)) %>% arrange(desc(mean_cv)))

cor_country <- inner_join(country, mean_dist, by="country") %>% mutate(distance = ifelse(country=="Africa", 0, distance)) %>% filter(country!="Africa")
...
(variability <- classification %>% filter(familyname %in% cn) %>% group_by(pop, sex, familyname, country, class) %>% summarise(mean_cn = mean(copynumber), variance = var(copynumber), coeff_var = variance/mean_cn, individuals = n()) %>% ungroup())

(pop <- variability %>% group_by(pop, sex, country, individuals, class) %>% summarise(mean_cv = mean(coeff_var, na.rm=TRUE), normalized_cv = mean_cv/individuals) %>% arrange(desc(mean_cv)) %>% distinct())

(country <- pop %>% group_by(country, sex, class) %>% summarise(mean_cv = mean(mean_cv, na.rm = TRUE), individuals=sum(individuals)) %>% arrange(desc(mean_cv)))
...
ggsave("/Users/rpianezza/Desktop/serial-class.png", copynumber_distance, dpi = 800, width = 10, height = 7)
```
