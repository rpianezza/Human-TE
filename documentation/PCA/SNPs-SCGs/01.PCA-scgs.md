...
- **HGDP_final** = only the GC-bias free samples in the HGDP, defined as
  ...
  neither \>0.5 or \<0.5, meaning that the estimated copynumber for the
...
- **distances** = kilometric distance from the out of Africa (Egypt) for
  all the population in the dataset
...
- **perc_var** = the minimum variance of the major allele frequency
  ...
  the firsts 2000 more variant SNPs \*\* **0.1219** = should include the
