...
hte=collections.defaultdict(lambda:0)
hscg=collections.defaultdict(lambda:0)
hkrab=collections.defaultdict(lambda:0)
hscgx=collections.defaultdict(lambda:0)
...
r2	0	M14653_te	240	70	27M	*	0	0	AACAGCTGCGGAATCGCACCGAATGCT	BBBBBFFFFFBFFFFFFFFFFFFFFFF	PG:Z:novoalign	AS:i:0	UQ:i:0	NM:i:0	MD:Z:27
     ...
     if flag & 0x004 > 0:   # remove unmapped
          continue
     summapped+=1 # all reads with sufficient mapping quality and 
...
print("{0}\t{1}\t{2}".format("summary","mapping_to_te",sumte))
print("{0}\t{1}\t{2}".format("summary","mapping_to_krab",sumkrab))
print("{0}\t{1}\t{2}".format("summary","mapping_to_scg",sumscg))
print("{0}\t{1}\t{2}".format("summary","mapping_to_scgx",sumscgx))
